...
template<class Grid, class Node >
KERNEL void K_resetValue_3D(Grid g, Node node)
{
...
public:
    ...
    // Initialisations node
    DEVICE_HOST MultiGrid& operator=(Node const& node) {
        MultiGrid::index_type idx;
        iterInit(idx);
        while (iterNext(idx)) {
            (*this)(idx, extents_strides) = node;
        }
        return *this;
    }

    DEVICE_HOST bool valideAndPositiveIndex(index_type& idx) {
        bool ret = true;
        size_t n = 0;
        while (n < Dim) {
            if (idx[n] < 0 || idx[n] >= extents[n]) {
                ret = false;
                break;
            }
          ++n;
    ...
    DEVICE_HOST bool valideIndex(index_type& idx) {
        bool ret = true;
        size_t n = 0;
        while (n < Dim) {
            if (idx[n] >= extents[n]) {
    ...
    DEVICE_HOST Node& operator()(index_type& idx, strides_type& str) const {
        size_t offset = 0;
    ...
    DEVICE_HOST Node& operator()(index_type& idx) {
        size_t offset = 0;
#if MG_ACCESS_ITERATIF
        {
          size_t n = 1;
          while (n != Dim) {
            offset += idx[n] * this->extents_strides[n];
            ++n;
          }
        }
#endif
    ...
    DEVICE_HOST Node& operator()(size_t offset) {
        return *((Node*)((char*)_data + offset));
    }

    template <typename MultiGrid>
    DEVICE_HOST GLint findRoot(MultiGrid& g_dstree, GLint offset)
    {
        GLint r = offset;
        while(r != g_dstree(r))
        {
    ...
    DEVICE_HOST GLint compute_offset(index_type idx) {
        GLint offset = 0;
        {
          size_t n = 1;
          while (n != Dim) {
        ...
        return offset + idx[0] * sizeof(Node);
    }

    template <typename Grid >
    DEVICE_HOST GLint translate_offset(Grid& g_ext, GLint idx) {
        return compute_offset(g_ext.back_offset(idx));
    }

    DEVICE_HOST index_type back_offset(GLint offset) {
        index_type bidx(0);
        {
          size_t n = Dim-1;
          while (n != 0) {
            GLint stride = this->extents_strides[n];
            bidx[n] = offset / stride;
            offset = offset % stride;
            --n;
          }
          bidx[0] = offset/sizeof(point_type);
        }
        return bidx;
    }
    ...
    DEVICE_HOST void resize(size_t w) {
        extents_type exts(1);
        exts[0] = w;
        if (Dim >= 2)
            exts[1] = 1;
    ...
    DEVICE_HOST void resize(size_t w, size_t h) {
        extents_type exts(1);
        exts[0] = w;
        if (Dim >= 2)
            exts[1] = h;
    ...
    DEVICE_HOST void resize(size_t w, size_t h, size_t d) {
        extents_type exts(1);
        exts[0] = w;
        if (Dim >= 2)
            exts[1] = h;
    ...
    void gpuResize(size_t w, size_t h) {
        extents_type exts(1);
        exts[0] = w;
        if (Dim >= 2)
            exts[1] = h;
    ...
    void gpuResize(size_t w, size_t h, size_t d) {
        extents_type exts(1);
        exts[0] = w;
        if (Dim >= 2)
            exts[1] = h;
    ...
    GLOBAL void gpuResetValue(Node const& node) {
        KER_CALL_THREAD_BLOCK_3D(b, t,
                              ...
                              this->getHeight(),
                              this->getDepth());
        K_resetValue_3D _KER_CALL_(b, t) ((*this), node);
    }
    ...
    // Input/Ouput
    friend ostream& operator<<(ostream & o, grid_type & mat) {
        if (!o)
            return(o);
        ...
        grid_type::index_type idx(0);
        mat.iterInit(idx);
        while (mat.iterNext(idx)) {
            o << mat(idx, mat.extents_strides) << " ";
            if (idx[0] == mat.width - 1)
                o << endl;
            if (idx[0] == mat.width - 1 && idx[1] == mat.height - 1)
                o << endl;
        }
        return o;
    }

    friend ifstream& operator>>(ifstream& i, grid_type& mat) {
        char str[256];

        if (!i)
            return(i);

        grid_type::extents_type exts;
        size_t Dimension = mat.getDimension();
...
//            is >> exts;
        ...
        while (mat.iterNext(idx)) {
            Node n;
            i >> n;
            mat(idx, mat.extents_strides) = n;
            //cout << "index " << idx << endl;
            //cout << "value " << mat(idx, mat.extents_strides) << endl;
        }
        return i;
    }
...
struct Grid : MultiGrid<Node, 2> {
  ...
  public:
    ...
    typedef typename super_type::strides_type strides_type;
    typedef typename super_type::coord_type coord_type;
    typedef typename super_type::iterator_type iterator_type;

    DEVICE_HOST explicit Grid() : super_type() {}
    DEVICE_HOST explicit Grid(int w, int h) : super_type(w, h) {}

    //! @brief ResetValue method on GPU side
    //! @brief ResetValue method on GPU side
    GLOBAL void gpuResetValue3D(Node const& node) {
        KER_CALL_THREAD_BLOCK_3D(b, t,
                              4, 4, 4,
                              this->getWidth(),
                              this->getHeight(),
    ...
    DEVICE_HOST Node& operator()(index_type& idx, strides_type& str) const {
        return super_type::operator ()(idx, str);
    }
    ...
    DEVICE_HOST Node& operator()(size_t idx) {
        return super_type::operator ()(idx);
    }
...
//    }
    ...
    DEVICE_HOST Grid& operator=(Node const& node) {
        return (Grid&)((super_type&) *this = node);
    }
...
//    }
    ...
    DEVICE_HOST inline Node const& fetchIntCoor(int x, int y) const {
        if (x < 0) x = abs(x + 1);
        if (y < 0) y = abs(y + 1);
        if (x >= this->width) x = this->width * 2 - x - 1;
        if (y >= this->height) y = this->height * 2 - y - 1;
        return *((Node*)((char*)this->_data + y * this->pitch) + x);
    }
    ...
    DEVICE_HOST inline void set(std::size_t const x, std::size_t const y, Node const& value) {
        *((Node*)((char*)this->_data + y * this->pitch) + x) = value;
    }
    ...
    DEVICE_HOST Grid& operator+=(Grid const& g2) {
        ...
        for (int _y = 0; _y < this->height; _y++) {
            for (int _x = 0; _x < this->width; _x++) {
                (*((Node*)((char*)this->_data + _y * this->pitch) + _x)) += g2.get(_x, _y);
            }
        }
        return *this;
    }
    ...
    DEVICE_HOST Grid& operator+=(Node const& g2) {
        ...
        for (int _y = 0; _y < this->height; _y++) {
            for (int _x = 0; _x < this->width; _x++) {
                (*((Node*)((char*)this->_data + _y * this->pitch) + _x)) += g2;
            }
        }
        return *this;
    }
    ...
    DEVICE_HOST inline Node const& fetchFloatCoor(GLfloat x, GLfloat y) const {
        ...
        int ix1 = ix0 + 1;
        int iy1 = iy0 + 1;
        if (ix1 >= this->width) ix1 = this->width * 2 - ix1 - 1;
        if (iy1 >= this->height) iy1 = this->height * 2 - iy1 - 1;
